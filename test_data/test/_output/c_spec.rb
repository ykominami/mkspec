...
RSpec.describe "tecsgen command", type: :aruba do
  ...
  let(:conf) { Mkspec::TestConf.new(ENV["MKSPEC_SPECIFIC_YAML_FNAME"], ENV["MKSPEC_GLOBAL_YAML_FNAME"], "mkspec", "tecsmerge", "E:/Crepo/ykominami/mkspec-rel/mkspec/test_data") }
  let(:o) { conf.o }
  ...
  context "cmoposite_1" do
    before(:each) {
      abs_test_case_parent_dir_pn = target_parent_dir_pn.join(test_case_parent_dir_pn)
      target_dir_pn = abs_test_case_parent_dir_pn.join("cmoposite_1")
      @cmdline = Clitest::Cmdline.new(nil, nil, target_dir_pn, "tecsgen")
    }
