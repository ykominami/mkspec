...
#@b_generate::bool  : true : TracePlugin を生成する必要がある
  ...
  def gen_plugin_decl_code( file )
    ...
    if @signature != nil then
      @signature.each_param do |fd, param|
...
#      file.print( "#{indent}syslog( LOG_INFO, \"#{indent}[#{direction}]#{type_str} #{outer}#{name}#{outer2} = %ld;\", (long)#{outer}#{name}#{outer2} );\n" )
    ...
    when StructType
      ...
      members_decl.get_items.each do |m|
...
#    "syslogCelltype"     => Proc.new { |obj,rhs| obj.set_syslogCelltype rhs },
    ...
    "maxArrayDisplay" => Proc.new { |obj, rhs| obj.set_max_srray_fisplay rhs },
    "cellEntry" => Proc.new { |obj, rhs| obj.set_cell_entry rhs },
    "probeName" => Proc.new { |obj, rhs| obj.set_probe_name rhs },
    "displayTime" => Proc.new { |obj, rhs| obj.set_display_time rhs },
    "kernelCelltype" => Proc.new { |obj, rhs| obj.set_kernel_celltype rhs },
    "syslogCelltype" => Proc.new { |obj, rhs| obj.set_syslog_celltype rhs },
  }

#  def set_maxArrayDisplay rhs
  def set_max_array_display rhs
    @maxArrayDisplay = rhs
  end

#  def set_cellEntry rhs
  def set_cell_entry rhs
...
#  def set_probeName rhs
  def set_probe_name rhs
    @probeName = rhs.to_s
  end

#  def set_displayTime rhs
  def set_display_time rhs
    if rhs.to_s == "true"
      @b_diplayTime = true
