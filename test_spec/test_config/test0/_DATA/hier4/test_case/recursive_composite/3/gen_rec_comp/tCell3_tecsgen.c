/*
 * This file was automatically generated by tecsgen.
 * This file is not intended to be edited.
 */
#include "tCell3_tecsgen.h"
#include "tCell3_factory.h"

/* entry port descriptor type #_EDT_# */
/* eEntry3 */
struct tag_tCell3_eEntry3_DES {
    const struct tag_sSig3_VMT *vmt;
    tCell3_IDX  idx;
    int_t  subscript;
};

/* entry port skelton function #_EPSF_# */
/* eEntry3 */
int32_t        tCell3_eEntry3_func1_skel( const struct tag_sSig3_VDES *epd, int32_t a)
{
    struct tag_tCell3_eEntry3_DES *lepd
        = (struct tag_tCell3_eEntry3_DES *)epd;
    return tCell3_eEntry3_func1( lepd->idx, lepd->subscript, a );
}
int32_t        tCell3_eEntry3_func2_skel( const struct tag_sSig3_VDES *epd, int32_t a)
{
    struct tag_tCell3_eEntry3_DES *lepd
        = (struct tag_tCell3_eEntry3_DES *)epd;
    return tCell3_eEntry3_func2( lepd->idx, lepd->subscript, a );
}

/* entry port skelton function table #_EPSFT_# */
/* eEntry3 */
const struct tag_sSig3_VMT tCell3_eEntry3_MT_ = {
    tCell3_eEntry3_func1_skel,
    tCell3_eEntry3_func2_skel,
};

/* entry port descriptor referenced by call port (differ from actual definition) #_CPEPD_# */
extern struct tag_sSig4_VDES comp1_comp12_cell4_eEntry4_des;
extern struct tag_sSigB_VDES comp1_comp11_cell2_eEntryB_des;

extern struct tag_sSig4_VDES comp2_comp12_cell4_eEntry4_des;
extern struct tag_sSigB_VDES comp2_comp11_cell2_eEntryB_des;

extern struct tag_sSig4_VDES rLargeComp_LargeComp_comp1_comp12_cell4_eEntry4_des;
extern struct tag_sSigB_VDES rLargeComp_LargeComp_comp1_comp11_cell2_eEntryB_des;

extern struct tag_sSig4_VDES rLargeComp_LargeComp_comp2_comp12_cell4_eEntry4_des;
extern struct tag_sSigB_VDES rLargeComp_LargeComp_comp2_comp11_cell2_eEntryB_des;

/* call port array #_CPA_# */




/* array of attr/var #_AVAI_# */
/* cell INIB #_INIB_# */
tCell3_INIB tCell3_INIB_tab[] = {
    /* cell: tCell3_CB_tab[0]:  comp1_comp12_cell3 id=1 */
    {
        /* call port (INIB) #_CP_# */ 
        &tCell4_INIB_tab[0],                     /* cCall3 #_CCP2_# */
        &tCell2_INIB_tab[0],                     /* cCallB #_CCP2_# */
        /* entry port #_EP_# */ 
        /* attribute(RO) */ 
        "comp1_comp12_cell3",                    /* name */
        30,                                      /* a */
    },
    /* cell: tCell3_CB_tab[1]:  comp2_comp12_cell3 id=2 */
    {
        /* call port (INIB) #_CP_# */ 
        &tCell4_INIB_tab[1],                     /* cCall3 #_CCP2_# */
        &tCell2_INIB_tab[1],                     /* cCallB #_CCP2_# */
        /* entry port #_EP_# */ 
        /* attribute(RO) */ 
        "comp2_comp12_cell3",                    /* name */
        30,                                      /* a */
    },
    /* cell: tCell3_CB_tab[2]:  LargeComp_comp1_comp12_cell3 id=3 */
    {
        /* call port (INIB) #_CP_# */ 
        &tCell4_INIB_tab[2],                     /* cCall3 #_CCP2_# */
        &tCell2_INIB_tab[2],                     /* cCallB #_CCP2_# */
        /* entry port #_EP_# */ 
        /* attribute(RO) */ 
        "LargeComp_comp1_comp12_cell3",          /* name */
        30,                                      /* a */
    },
    /* cell: tCell3_CB_tab[3]:  LargeComp_comp2_comp12_cell3 id=4 */
    {
        /* call port (INIB) #_CP_# */ 
        &tCell4_INIB_tab[3],                     /* cCall3 #_CCP2_# */
        &tCell2_INIB_tab[3],                     /* cCallB #_CCP2_# */
        /* entry port #_EP_# */ 
        /* attribute(RO) */ 
        "LargeComp_comp2_comp12_cell3",          /* name */
        30,                                      /* a */
    },
};

/* entry port descriptor #_EPD_# */
extern const struct tag_tCell3_eEntry3_DES comp1_comp12_cell3_eEntry3_des0;
const struct tag_tCell3_eEntry3_DES comp1_comp12_cell3_eEntry3_des0 = {
    &tCell3_eEntry3_MT_,
    &tCell3_INIB_tab[0],      /* INIB 1 */
    0
};
extern const struct tag_tCell3_eEntry3_DES comp1_comp12_cell3_eEntry3_des1;
const struct tag_tCell3_eEntry3_DES comp1_comp12_cell3_eEntry3_des1 = {
    &tCell3_eEntry3_MT_,
    &tCell3_INIB_tab[0],      /* INIB 1 */
    1
};
extern const struct tag_tCell3_eEntry3_DES comp2_comp12_cell3_eEntry3_des0;
const struct tag_tCell3_eEntry3_DES comp2_comp12_cell3_eEntry3_des0 = {
    &tCell3_eEntry3_MT_,
    &tCell3_INIB_tab[1],      /* INIB 1 */
    0
};
extern const struct tag_tCell3_eEntry3_DES comp2_comp12_cell3_eEntry3_des1;
const struct tag_tCell3_eEntry3_DES comp2_comp12_cell3_eEntry3_des1 = {
    &tCell3_eEntry3_MT_,
    &tCell3_INIB_tab[1],      /* INIB 1 */
    1
};
extern const struct tag_tCell3_eEntry3_DES rLargeComp_LargeComp_comp1_comp12_cell3_eEntry3_des0;
const struct tag_tCell3_eEntry3_DES rLargeComp_LargeComp_comp1_comp12_cell3_eEntry3_des0 = {
    &tCell3_eEntry3_MT_,
    &tCell3_INIB_tab[2],      /* INIB 1 */
    0
};
extern const struct tag_tCell3_eEntry3_DES rLargeComp_LargeComp_comp1_comp12_cell3_eEntry3_des1;
const struct tag_tCell3_eEntry3_DES rLargeComp_LargeComp_comp1_comp12_cell3_eEntry3_des1 = {
    &tCell3_eEntry3_MT_,
    &tCell3_INIB_tab[2],      /* INIB 1 */
    1
};
extern const struct tag_tCell3_eEntry3_DES rLargeComp_LargeComp_comp2_comp12_cell3_eEntry3_des0;
const struct tag_tCell3_eEntry3_DES rLargeComp_LargeComp_comp2_comp12_cell3_eEntry3_des0 = {
    &tCell3_eEntry3_MT_,
    &tCell3_INIB_tab[3],      /* INIB 1 */
    0
};
extern const struct tag_tCell3_eEntry3_DES rLargeComp_LargeComp_comp2_comp12_cell3_eEntry3_des1;
const struct tag_tCell3_eEntry3_DES rLargeComp_LargeComp_comp2_comp12_cell3_eEntry3_des1 = {
    &tCell3_eEntry3_MT_,
    &tCell3_INIB_tab[3],      /* INIB 1 */
    1
};
